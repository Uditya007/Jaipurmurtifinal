        ...
        <div className="glass rounded-3xl p-8 relative overflow-hidden">
          ...
          {!showForgotPassword && (
            <>
              ...
              {/* ── Bottom: Or continue with ── */}
              {isAuthTab && !otpSent && authMethod === 'email' && (
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button type="button"
                      ...
                      className="flex flex-col items-center justify-center gap-2.5 py-3.5 rounded-2xl border border-gold/15 text-muted hover:border-gold/40 hover:text-divine hover:bg-gold/5 transition-all duration-200">
                      <Phone size={18} />
                      <span className="text-[10px] tracking-widest uppercase">Phone OTP</span>
                    </button>
                    ...
                    <button type="button"
                      ...
                      className="flex flex-col items-center justify-center gap-2.5 py-3.5 rounded-2xl bg-white text-gray-700 hover:bg-gray-50 transition-all duration-200 shadow-sm">
                      {googleLoading ? <Loader2 size={18} className="animate-spin text-gray-500" /> : (
                        <>
                          ...
                          <span className="text-[10px] tracking-widest uppercase text-gray-600">Google</span>
                        </>
                      )}
                    </button>
                  </div>
              ...
              {/* Phone form (when phone method selected) */}
                  ...
                  <AnimatePresence mode="wait">
                    {!otpSent ? (
                      <motion.form key="phone-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <p className="text-muted text-xs text-center">Enter your phone number. We will send a 6-digit OTP via SMS.</p>
                        <div className="relative flex items-center">
                          ...
                          <input type="tel" placeholder="98765 43210" required
                            value={form.phone} onChange={(e) => handleUpdate('phone', e.target.value)}
                        ...
                        <button type="button" onClick={() => { setAuthMethod('email'); setErrorMsg(''); }}
                          className="w-full text-center text-xs text-muted hover:text-gold transition-colors pt-1">← Back to Email</button>
                      </motion.form>
                    ) : (
                      <motion.form key="otp-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        onSubmit={handleVerifyOtp} className="space-y-4">
                        <p className="text-muted text-xs text-center">Enter the 6-digit OTP sent to +91 {form.phone}</p>
                        <div className="relative">
                        ...
                        <button type="button" onClick={() => { setOtpSent(false); setOtp(''); setErrorMsg(''); }}
                          className="w-full text-center text-xs text-muted hover:text-gold transition-colors pt-1">← Change phone number</button>
                      </motion.form>
                    )}
                  </AnimatePresence>
                </>
